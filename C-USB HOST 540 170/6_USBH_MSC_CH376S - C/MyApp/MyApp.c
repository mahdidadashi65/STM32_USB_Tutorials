...
void loop() 
{
  ...
  if(st==HAL_OK)
    ...
    if(((tmpCommand > 48)&&(tmpCommand < 58)) && !Ch376msc_checkDrive()){ // if the data is ASCII 0 - 9 and no flash drive are attached
     ...
     switch (tmpCommand) {
      ...
      case 49: //1
        printInfo("COMMAND1: Create and write data to file : TEST1.TXT");    // Create a file called TEST1.TXT
          Ch376msc_setFileName("TEST1.TXT");  //set the file name
          Ch376msc_openFile();                //open the file

          for(int a = 0; a < 20; a++){          //write text from string(adat) to flash drive 20 times
            Ch376msc_writeFile(adat, strlen(adat)); //string, string length
          }
          Ch376msc_closeFile();               //at the end, close the file
        printInfo("Done!");
        break;
//*****************************************************************************************************************************************************
      case 50: //2
        ...
        Ch376msc_setFileName("TEST1.TXT");  //set the file name
        Ch376msc_openFile();                //open the file
        Ch376msc_moveCursor(CURSOREND);     //move the "virtual" cursor at end of the file, with CURSORBEGIN we actually rewrite our old file
        //Ch376msc_moveCursor(Ch376msc_getFileSize()); // is almost the same as CURSOREND, because we put our cursor at end of the file

        for(int a = 0; a < 20; a++){          //write text from string(adat) to flash drive 20 times
        	if(Ch376msc_getFreeSectors()){ //check the free space on the drive
        		Ch376msc_writeFile(adat2, strlen(adat2)); //string, string length
        	} else {
        		printf("Disk full\r\n");
        	}
        }
...
//*****************************************************************************************************************************************************
      case 51: //3
		...
		{ // our temporary buffer where we read data from flash drive and the size of that buffer
        	readMore = Ch376msc_readFile(adatBuffer, sizeof(adatBuffer));
...
//*****************************************************************************************************************************************************
      case 52: //4
        ...
        Ch376msc_openFile();                          //open the file
          ...
          printf("%s\r\n",Ch376msc_getFileName());
          printf("%d",Ch376msc_getYear());
          printf("y\t");
          printf("%d",Ch376msc_getMonth());
          printf("m\t");
          printf("%d",Ch376msc_getDay());
          printf("d\t");
          printf("%d",Ch376msc_getHour());
          printf("h\t");
          printf("%d",Ch376msc_getMinute());
          printf("m\t");
          printf("%d",Ch376msc_getSecond());
          printf("s\r\n");
        Ch376msc_closeFile();                         //at the end, close the file
        printInfo("Done!");
        break;
//*****************************************************************************************************************************************************
      case 53: //5
        ...
        Ch376msc_openFile();                //open the file
          ...
          Ch376msc_setYear(2019);
          Ch376msc_setMonth(2);
          Ch376msc_setDay(24);
          Ch376msc_setHour(15);
          Ch376msc_setMinute(47);
          Ch376msc_setSecond(26);

          Ch376msc_dirInfoSave();           //save the changed data
        Ch376msc_closeFile();               //and yes again, close the file after when you don`t use it
        printInfo("Done!");
        break;
//*****************************************************************************************************************************************************
      case 54: //6
        ...
        Ch376msc_setFileName("TEST1.TXT");  //set the file name
        Ch376msc_deleteFile();              //delete file
        printInfo("Done!");
        break;
//*****************************************************************************************************************************************************
      case 55: //7
        printInfo("COMMAND7: List root directory");                          //Print all file names in the current directory
          while(Ch376msc_listDir()){ // reading next file
            printf("%s",Ch376msc_getFileName()); // get the actual file name
            printf(" : ");
             printf("%d",Ch376msc_getFileSize()); // get the actual file size in bytes
            printf(" >>>\t");
             printf("%d\r\n",Ch376msc_getFileSizeStr()); // get the actual file size in formatted string
          }
          printInfo("Done!");
        break;
//*****************************************************************************************************************************************************
      case 56: //8
    	  totSect = Ch376msc_getTotalSectors(); // get the total sector number
    	  freeSect = Ch376msc_getFreeSectors(); // get the available sector number
...
//    	  }
    	  ...
    	  printf("%d",percentg);
    	  printf("%%");
    	  switch (Ch376msc_getFileSystem()) { //1-FAT12, 2-FAT16, 3-FAT32
			case 1:
